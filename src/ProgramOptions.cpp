...
Config ProgramOptions::parseCommandLine(int argc, char* argv[])
{
    try
    {
        ...
        options.add_options()
            ...
            ("secondary-font-file", "path to ttf file, required", cxxopts::value<std::string>(config.secondaryFontFile))
            ...
            ("force-auto-hinter", "use a lighter hinting algorithm", cxxopts::value<bool>(config.forceAutoHinter))
